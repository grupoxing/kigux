...
var FancyUpload2 = new Class({
	...
	render: function() {
        ...
        this.currentTemp  = this.status.getElement('.current-temp');
	...
	onProgress: function(file, current, overall) {
		...
		this.currentText.set('html', 'Enviando com {rate}/s.'.substitute({
			rate: (current.rate) ? this.sizeToKB(current.rate) : '- B'
		}));
		this.currentTemp.set('html', 'Tempo estimado: ~{timeLeft}'.substitute({
